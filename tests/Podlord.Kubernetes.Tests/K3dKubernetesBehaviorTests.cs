...
public sealed class K3dKubernetesBehaviorTests
{
    ...
    public async Task Detail_and_log_identity_requests_fill_identity_caches_through_real_queue()
    {
        ...
        Assert.Contains("podlord-log-ready", logSnapshot.Text, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("podlord-log-ready", service.GetCachedPodLogs(logs)?.Text, StringComparison.OrdinalIgnoreCase);
    }
    ...
    public async Task Pod_logs_fetch_real_tail_from_running_pod()
    {
        ...
        Assert.Contains("podlord-log-ready", logs.Text, StringComparison.OrdinalIgnoreCase);
    }
...
public sealed class K3dClusterFixture : IAsyncLifetime
{
    ...
    {
        await Run("kubectl", ["--kubeconfig", KubeconfigPath, "-n", "payments", "rollout", "status", "deployment/podlord-healthy", "--timeout=240s"], TimeSpan.FromMinutes(5));
        await Run("kubectl", ["--kubeconfig", KubeconfigPath, "-n", "payments", "wait", "--for=condition=Ready", "pod", "-l", "app=podlord-log", "--timeout=240s"], TimeSpan.FromMinutes(5));
        await Run("kubectl", ["--kubeconfig", KubeconfigPath, "-n", "batch", "wait", "--for=condition=complete", "job/podlord-success", "--timeout=240s"], TimeSpan.FromMinutes(5));
        await Run("kubectl", ["--kubeconfig", KubeconfigPath, "-n", "batch", "wait", "--for=condition=failed", "job/podlord-fail", "--timeout=240s"], TimeSpan.FromMinutes(5));
        await WaitForOutput(
            "kubectl",
